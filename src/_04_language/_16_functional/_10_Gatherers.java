 ...
 *   stream behavior.
 * - Combine with terminal operations like toList(), forEach(), or collect() to extract results from the transformed
 *   stream.
 */
void main() {
    /*
     ...
     * - If the total number of elements is not divisible by the window size, the last window may contain fewer
     *   elements.
     ...
     * - "Gatherers.fold(initialSupplier, accumulator)" creates a gatherer that reduces all stream elements into a
     *   single accumulated value.
     * - The initial value is provided by "initialSupplier", and each element is combined with the accumulator function
     *   to update the accumulated state.
     * - Unlike "scan", fold emits only one element representing the final accumulation after processing the entire
     *   stream.
     ...
     * - Useful for performing CPU-bound or IO-bound transformations concurrently while leveraging Java's virtual
     *   threads.
