/*
 * Collectors
 * - This section explains the "Collectors" utility class in Java, which provides static methods to create common
 *   Collector instances for use with the Stream API, including collection, reduction, summarization, grouping, and
 ...
 *   maxBy().
 * - Supports advanced grouping and partitioning operations with groupingBy(), groupingByConcurrent() and
 *   partitioningBy().
...
void main() {
    ...
    /*
     * Collect To Map
     * - "Collectors.toMap()" collects the elements of a stream into a Map by specifying key and value mapping
     *   functions.
     ...
     * - If an unmodifiable map is desired, "Collectors.toUnmodifiableMap()" can be used, which returns a Map that
     *   throws UnsupportedOperationException on modification attempts.
     * - For parallel streams or concurrent scenarios, "Collectors.toConcurrentMap()" can be used to produce a
     *   thread-safe Map.
    ...
    /*
     * Reduce to Sum Value
     * - "Collectors.summingInt()" performs a summing reduction on the elements of a stream using an int-valued
     *   function.
     * - The collector applies the function to each element and returns the total sum as an int.
     * - Related collectors include "Collectors.summingLong()" for long values and "Collectors.summingDouble()" for
     *   double values.
     ...
     * - "Collectors.averagingDouble()" performs an averaging reduction on the elements of a stream using a
     ...
     * - The first argument is the identity (initial value) for the reduction, and the second is the accumulator
     *   function that combines the current result with each element.
     ...
     * - "Collectors.mapping()" applies a mapping function to each element of the stream and collects the results using
     ...
     * - Useful for combining transformation and collection in a single terminal operation, avoiding intermediate
     *   streams.
     * - Works with both sequential and parallel streams.
     ...
     * - "Collectors.flatMapping()" applies a function to each element of the stream that produces a stream, then
     *   flattens all resulting streams into a single stream before collecting with a downstream collector.
     ...
     * - The resulting Map has keys of type K (from the classifier) and value of type List<V> by default
     *   (Map<K, List<V>>).
     ...
     * - "Collectors.collectingAndThen()" performs a collection using a downstream collector and then applies a
     *   finishing function to the result.
     ...
     * - "Collector.of()" allows creating a fully custom collector by specifying the supplier, accumulator, combiner,
     *   and finisher functions.
     * - Supplier: provides a new mutable container (here, ArrayList<String>).
     * - Accumulator: defines how each element is added to the container (here, ArrayList::add). The first argument of
     *   the BiConsumer becomes the container, and the second is the element from the stream.
     * - Combiner: defines how two containers are merged in parallel streams (here, adding all elements from one list
     *   into another and returning the first list).
