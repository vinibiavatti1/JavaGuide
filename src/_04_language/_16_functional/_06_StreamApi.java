 ...
 * - The Stream API is provided by the Collection framework; streams are available for collections, and once created,
 *   the same Stream API applies regardless of the collection type, differing only by element type and source
 *   characteristics.
 *
 ...
 * - These streams offer additional numeric operations such as: sum(), average(), min(), max(), summaryStatistics(),
 *   etc.
 * - Conversion methods exist between object streams and primitive streams: mapToInt(...), mapToLong(...),
 *   mapToDouble(...), boxed().
 *
 * Notes:
...
void main() {
     ...
     * - Unlike "collect(Collectors.toList())", the List returned by "toList()" is unmodifiable (attempting to
     *   add/remove elements will throw an exception).
     * - Output: [A, B, C]
     */
     ...
     * - The "toArray()" method is a terminal operation that collects the elements of the stream into a new Object
     *   array.
     * - It is useful when you need to convert a stream into an array for interoperation with APIs that require arrays.
     * - There is also an overloaded version "toArray(IntFunction<A[]> generator)" that allows you to create a typed
     *   array (e.g., String[], Integer[]).
    ...
    /*
     * Check if Any Match
     * - The "anyMatch" method is a terminal operation that returns true if at least one element of the stream matches
     *   the given predicate.
     ...
     * - In sequential streams, it typically returns the first element, but in parallel streams it may return any
     *   element for better performance.
     * - Returns an Optional, so methods like "ifPresent", "orElse", or "orElseThrow" can be used to safely handle the
     *   result.
     ...
     * - Useful for quickly determining the size of a stream, especially after filtering or other intermediate
     *   operations.
     * - Works with both sequential and parallel streams.
     * - Output: 3
     */
    ...
    /*
     * Filter
     * - The "filter" method is an intermediate operation that returns a new stream consisting of elements that match
     *   the given predicate.
    ...
    /*
     * Map
     * - The "map" method is an intermediate operation that returns a new stream consisting of the results of applying
     *   the given function to each element of the original stream.
    ...
    /*
     * Map Multi
     * - The "mapMulti" method is an intermediate operation that allows each element of the stream to produce zero, one,
     *   or multiple elements in the resulting stream.
    ...
    /*
     * Flat Map
     * - The "flatMap" method is an intermediate operation that replaces each element of the stream with a stream
     *   produced by a provided mapping function, then flattens the resulting streams into a single stream.
     * - Useful for flattening nested structures, such as lists of lists, or for expanding elements into multiple
     *   outputs.
     * - Works with both sequential and parallel streams.
     * - Related methods: "map" (produces exactly one output per input) and "mapMulti" (can produce zero, one, or
     *   multiple outputs per input without creating nested streams).
     * - Output: [A, B, C, D]
     */
    ...
    /*
     * Distinct
     * - The "distinct" method is an intermediate operation that returns a stream consisting of the unique elements of
     *   the original stream, removing duplicates.
    ...
    /*
     * Limit
     * - The "limit" method is an intermediate operation that returns a stream consisting of no more than the given
     *   number of elements from the original stream.
    ...
    /*
     * Skip
     * - The "skip" method is an intermediate operation that returns a stream consisting of the remaining elements of
     *   the original stream after discarding the first n elements.
    ...
    /*
     * Take While
     * - The "takeWhile" method is an intermediate operation that returns a stream consisting of elements from the start
     *   of the original stream while the given predicate remains true.
    ...
    /*
     * Unordered
     * - The "unordered" method is an intermediate operation that signals that the stream's encounter order can be
     *   ignored.
    ...
    /*
     * Peek
     * - The "peek" method is an intermediate operation that allows performing an action on each element of the stream
     *   as it passes through, mainly for debugging or logging purposes.
    ...
    /*
     * Map to Int
     * - The "mapToInt" method is an intermediate operation that converts a Stream of objects into an IntStream by
     *   applying a function that produces an int for each element.
     ...
     * - Works with both sequential and parallel streams.
     * - In this example, the IntStream is consumed by the terminal operation "sum()" to produce the total of all
     *   elements.
     * - Output: 6
     */
    sum = Stream.of(1, 2, 3)
     ...
     * - The "boxed" method converts the primitive LongStream back into a Stream<Long>, which allows collection into
     *   Lists or use with APIs requiring objects.
     * - Works with both sequential and parallel streams.
     ...
     * - The "boxed" method can be used to convert the resulting IntStream into a Stream<Integer> for collection into
     *   Lists or use with APIs requiring objects.
     * - Works with both sequential and parallel streams.
     * - In this example, the IntStream is consumed by the terminal operation "sum()" to produce the total of all
     *   elements.
    ...
    /*
     * Flat Map To Long
     * - The "flatMapToLong" method is an intermediate operation that replaces each element of a Stream with a
     *   LongStream produced by a provided mapping function, then flattens the resulting LongStreams into a single
     *   LongStream.
     ...
     * - The "boxed" method can be used to convert the resulting LongStream into a Stream<Long> for collection into
     *   Lists or use with APIs requiring objects.
     * - Works with both sequential and parallel streams.
     * - In this example, the LongStream is consumed by the terminal operation "sum()" to produce the total of all
     *   elements.
    ...
    /*
     * Map Multi to Long
     * - The "mapMultiToLong" method is an intermediate operation that allows mapping each element of a Stream to zero
     *   or more long values using a BiConsumer, then flattens them into a single LongStream.
     * - Useful for expanding elements into multiple long values, performing numeric transformations, or preparing data
     *   for aggregation operations such as sum, average, min, or max.
     * - Related methods: "mapMulti" (object stream), "mapMultiToInt", and "mapMultiToDouble".
     * - In this example, the single element is emitted three times via "consumer.accept", and the resulting LongStream
     *   is reduced using the terminal operation "sum()".
     * - Works with both sequential and parallel streams.
     * - Output: 3
     */
    longSum = Stream.of(1)
