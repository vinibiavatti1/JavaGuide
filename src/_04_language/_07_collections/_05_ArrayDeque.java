...
void main() {
     ...
     * - By creating an ArrayDeque from another collection (such as List.of), you obtain a mutable double-ended queue
     *   that supports adding and removing elements from both the front and the back.
     * - Use this approach when you need an initial set of elements but want to modify the deque dynamically during
     *   program execution, using FIFO or LIFO semantics as required.
     * - Unlike List.of(...) which returns an immutable list, there is no built-in immutable variant of ArrayDeque in
     *   Java.
     ...
     * - It is used when treating the deque as a FIFO queue. Unlike add(), it does not throw an exception if the deque
     *   is full (not relevant for ArrayDeque, which resizes dynamically), instead returning false if insertion fails.
     * - Output: [A, B, C]
     */
     ...
     * - Unlike arrays, deque do not have a fixed length, and the size can change as elements are added or removed from
     *   either end.
     * - Output: 3
     */
    deque = new ArrayDeque<>(List.of("A", "B", "C"));
     ...
     * - The "peekLast()" method retrieves the last element without removing it, also returning null if the deque is
     *   empty.
    ...
    /*
     * Removing Elements
     * - The "remove(Object o)" method removes the first occurrence of the specified element from the Deque, if it
     *   exists.
     ...
     * - Throws NoSuchElementException if the deque is empty.
     * - Use this method when you want to remove elements from the front, following FIFO or stack LIFO semantics
     *   depending on how elements were added.
     * - Output: [B, C]
     */
    deque = new ArrayDeque<>(List.of("A", "B", "C"));
    deque.removeFirst();
     ...
     * - Throws NoSuchElementException if the deque is empty.
     * - Use this method when you want to remove elements from the back, following FIFO or stack LIFO semantics
     *   depending on how elements were added.
     * - Output: [A, B]
     */
    deque = new ArrayDeque<>(List.of("A", "B", "C"));
     ...
     * - The "removeFirstOccurrence(Object o)" method removes the first occurrence of the specified element from the
     *   Deque, searching from the front to the back.
     ...
     * - The "removeLastOccurrence(Object o)" method removes the last occurrence of the specified element from the
     *   Deque, searching from the back to the front.
     * - Returns true if the element was found and removed, false otherwise.
     * - Output: [A, B]
     */
    deque = new ArrayDeque<>(List.of("A", "B", "A"));
     ...
     * - This operation effectively performs an intersection between the two collections and modifies the original
     *   deque.
     * - Output: [A, B]
     */
    deque = new ArrayDeque<>(List.of("A", "B", "C"));
     ...
     * - The "containsAll(Collection<?> c)" method checks whether the Deque contains all elements from another
     *   collection.
