/*
 * Executors
 * - This section explains the Executors factory class in Java, which provides factory methods to create various types
 *   of ExecutorService implementations for managing and executing concurrent tasks efficiently.
 *
 * Overview
 ...
 *
 * CPU Bound
 * - Tasks whose performance is primarily limited by the processor's speed, such as complex calculations, data
 *   processing, or algorithmic operations. The CPU is continuously active, and the bottleneck is computation rather
 *   than I/O.
 * - These scenarios are best handled with Fixed Thread Pools, which leverage multiple OS threads to perform
 *   computations in parallel, but are more resource-intensive than virtual threads.
 *
 * Usage
...
void main() {
     ...
     * - Recommended for high-concurrency applications that perform blocking calls such as I/O, database access, or
     *   network requests.
     ...
     * - Useful when tasks must not run concurrently or when shared state must be accessed safely without
     *   synchronization.
     ...
     * - Useful for simple scheduling scenarios such as delayed execution, periodic maintenance, or lightweight
     *   cron-like jobs.
     ...
     * - Suitable for large-scale parallel computations, batch processing, or workloads that benefit from fine-grained
     *   task decomposition.
     * - Output: Hello World (printed by one of the pool threads)
     */
