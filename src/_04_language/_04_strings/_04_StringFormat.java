 ...
 * - Supports alignment, padding, numeric signs, locale-specific separators, and various data conversions.
 *
 * Syntax
 * - %[arg$][flags][width][.precision]conversion
 *
 * Placeholders:
 * - arg$:       (opt) Specifies the index of the argument to use (1-based). Useful for reordering arguments.
 * - flags:      (opt) Modify the formatting behavior, such as alignment, padding, sign display, or alternate forms.
 * - width:      (opt) Sets the minimum number of characters to be written. Padding is applied if necessary.
 * - .precision: (opt) Controls the number of digits or characters to display, depending on the conversion type.
 * - conversion: (req) Defines the type of data to format (e.g., s for strings, d for integers, f for floating-point).
 *
 * Flags:
