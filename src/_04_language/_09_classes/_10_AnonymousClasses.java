...
- Anonymous classes are less reusable but very convenient for small tasks that do not require a full class declaration.
- They can access members of their enclosing class and effectively final local variables from the enclosing scope.
- The JVM generates a class file for each anonymous class, named with "$" followed by a number (e.g., "Outer$1.class").
...
- Reflection metadata:
  ...
  - "getEnclosingMethod()" or "getEnclosingConstructor()" returns the method or constructor in which the anonymous class
    is defined.
...
- Avoid using anonymous classes for complex logic or reusable functionality; in such cases, use named inner or top-level
  classes.
