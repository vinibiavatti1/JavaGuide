...
Overview
- Concurrency is essential for applications that handle multiple users, perform I/O operations, or execute CPU-intensive
  tasks in parallel.
...
- Parallelism is a form of concurrency focused on executing CPU-bound computations simultaneously to reduce processing
  time.
...
- Synchronization and coordination mechanisms prevent race conditions and ensure data consistency when multiple threads
...
- Semaphore: concurrency control mechanism that limits the number of threads that can access a resource simultaneously,
  useful for throttling, connection pools, and shared resources.
