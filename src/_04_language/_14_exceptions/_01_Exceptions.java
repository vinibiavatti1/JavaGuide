 ...
 * - Java distinguishes between checked exceptions (must be declared or handled) and unchecked exceptions (do not need
 *   to be declared).
 * - Exceptions can be thrown using the keyword "throw". Ex: throw new RuntimeException();
 *
 * Key Characteristics
 ...
 *   - Runtime Exception: Unchecked exceptions (RuntimeException and its subclasses) do not need to be declared or
 *     caught.
 ...
 * - java.lang.IllegalStateException            Object is in an invalid state (ex: close an already closed connection)
