 ...
 *
 * Key Characteristics
 * - Provides methods to safely retrieve values (orElse, orElseGet, orElseThrow) and check presence (isPresent,
 *   isEmpty).
 ...
 *
 * Notes
 * - Optional is not recommended for fields, method parameters, or elements in collections, as it can hurt performance
...
void main() {
     ...
     * - Useful for avoiding null checks and enabling functional-style operations on values that may or may not be
     *   present.
     ...
     * - Note: The "get" method can also retrieve the value but is considered legacy because it throws exceptions
     *   directly without clear intent.
     ...
     * - The "or" method returns the current Optional if it contains a value, or a Supplier-provided alternative
     *   Optional if empty.
     ...
     * - The "filter" method returns an Optional describing the value if it matches a given predicate, otherwise an
     *   empty Optional.
