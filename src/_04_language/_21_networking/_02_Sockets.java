...
- TCP Sockets are ideal for applications that require reliability, ordered delivery, and a persistent connection, such
  as web servers, email, file transfers, and databases.
...
- Streams are the standard abstraction for reading and writing data; text or binary protocols can be implemented using
  these streams.
...
Server
- Demonstrates a minimal TCP server that listens on port 8089, accepts a client connection, and exchanges text messages
  using buffered I/O streams.
*/
...
/*
Client
- Demonstrates a minimal TCP client that connects to the server on localhost:8089, sends a text message, and reads the
  server's response.
*/
