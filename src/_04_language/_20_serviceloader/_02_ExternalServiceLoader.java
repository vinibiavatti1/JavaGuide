...
- This section explains how to use ServiceLoader to discover and load service implementations from external JARs at
  runtime using a custom ClassLoader. This approach enables dynamic plugin loading without compile-time dependencies on
  the provider implementations.

Overview
- External ServiceLoader builds on the standard ServiceLoader mechanism but allows providers to reside in JARs outside
  the main application classpath.
